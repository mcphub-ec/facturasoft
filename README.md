...
Servidor Model Context Protocol (MCP) para la integración con **FacturaSoft**.
...
-   Conector para el sistema de emisión de comprobantes FacturaSoft.
...
-   `emitir_factura`: Genera la factura en el ecosistema FacturaSoft.
...
```env
FACTURASOFT_API_KEY="tu_api_key_aqui"
```
...
```bash
docker run -d \
  --name mcp-facturasoft \
  --env-file .env \
  ghcr.io/mcphub-ec/mcp-facturasoft:latest
```
...
services:
  mcp-facturasoft:
    image: ghcr.io/mcphub-ec/mcp-facturasoft:latest
    container_name: mcp-facturasoft
    env_file:
      - .env
